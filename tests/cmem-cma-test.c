...
static void* map_dma_buffer(int fd, int buffer_id, size_t size)
{
    void *mapped_addr;
    off_t offset = buffer_id * getpagesize();  // Use buffer_id as offset
...
static int test_memory_access(void *mapped_addr, size_t size, int buffer_id)
{
    ...
    // Test byte access at boundaries
...
static int test_allocation_with_access(int fd, size_t size, int numa_node)
{
    ...
    printf("  User Address: 0x%lx\n", alloc_req.user_addr);

    // Map the buffer for userspace access
    mapped_addr = map_dma_buffer(fd, alloc_req.buffer_id, size);
    if (!mapped_addr) {
        ret = -1;
        goto cleanup;
    }
...
static int comprehensive_test(int fd)
{
    ...
    // Test allocations on different NUMA nodes
    for (int node = -1; node < numa_nodes && allocated_count < MAX_TEST_BUFFERS; node++) {
        for (int i = 0; i < num_sizes && allocated_count < MAX_TEST_BUFFERS; i++) {
            ...
            buffers[allocated_count].user_addr = req.user_addr;

            // Map buffer for userspace access
            buffers[allocated_count].mapped_addr = map_dma_buffer(fd, req.buffer_id, req.size);
    ...
    // Test memory access on all mapped buffers
...
static int memory_benchmark_test(int fd)
{
    ...
    for (int i = 0; i < num_sizes; i++) {
        ...
        mapped_addr = map_dma_buffer(fd, req.buffer_id, req.size);
        if (!mapped_addr) {
