...
static int comprehensive_test(int fd)
{
    ...
    for (int node = -1; node < numa_nodes && allocated_count < MAX_TEST_BUFFERS; node++) {
        for (int i = 0; i < num_sizes && allocated_count < MAX_TEST_BUFFERS; i++) {
            ...
            printf("Test %d: Allocating %u bytes on node %d... ", allocated_count + 1, req.size, node);
            ...
            buffers[allocated_count].mapped_addr = map_dma_buffer(fd, req.buffer_id, req.size, req.mmap_offset);
    ...
    for (int i = 0; i < allocated_count; i++) {
        ...
        printf("Freeing buffer %d (size %lu)... ", buffers[i].buffer_id, buffers[i].size);
...
int main(int argc, char* argv[])
{
    ...
    if (fd < 0) {
        ...
        fprintf(stderr, "Run this test as root, e.g. 'sudo %s -t'\n", g_prog_name);
        return 1;
    }
