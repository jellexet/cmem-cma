...
#define MAX_BUFFERS 16           /**< Maximum number of concurrently allocated buffers */

/**
 * @def CMEM_CMA_IOC_MAGIC
 * @brief IOCTL magic number used for CMA DMA commands
 *
 * 'C' is chosen arbitrarily but it simply must be unique among all IOCTL command groups.
 * 'C' is used by soundcard.h or capi.h, which should not create any problems.
 ...
 *
 * Userspace initializes `size` and optionally `numa_node`. Kernel fills in the rest.
 */
struct cmem_cma_alloc_req {
